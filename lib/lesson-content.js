...
function loadStaticPilotContent(lessonId) {
  try {
    const content = require(path.join('..', 'data', 'lesson-content', 'geography-lesson-1.json'));
    return Number(content.lesson_id) === Number(lessonId) ? content : null;
  } catch (_error) {
    return null;
  }
}
