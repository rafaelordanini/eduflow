const { deduplicateCurriculumLessons } = require('./lesson-deduplication');

const MACRO_PLAN_VERSION = 6;
...
function buildCurriculum(subjects, lessons) {
  ...
  deduplicateCurriculumLessons(lessons).forEach(function(lesson) {
...
function shiftIsoDate(value, deltaDays) {
  ...
  return date ? addDays(date, deltaDays) : value;
}

function flattenPlanItemsWithWeek(plan) {
  const rows = [];
  (plan && plan.semanas || []).forEach(function(week, weekIndex) {
    (week.materias || []).forEach(function(item, itemIndex) {
      rows.push({ week, weekIndex, item, itemIndex });
    });
  });
  return rows;
}

function getSequentialStudyDate(plan, requestedDate) {
  const targetDate = isoDate(requestedDate) || todayIso();
  const pendingStudy = flattenPlanItemsWithWeek(plan)
    .map(function(row) { return row.item; })
    .filter(function(item) { return item && item.tipo === 'estudo' && !item.done && isoDate(item.data); })
    .sort(function(a, b) {
      return String(a.data).localeCompare(String(b.data)) || Number(a.dia || 0) - Number(b.dia || 0);
    });
  if (pendingStudy.length && pendingStudy[0].data <= targetDate) return pendingStudy[0].data;
  return targetDate;
}

function rescheduleMacroPlanFromPendingStudy(plan, requestedDate) {
  const targetDate = isoDate(requestedDate) || todayIso();
  const sourceDate = getSequentialStudyDate(plan, targetDate);
  if (!plan || sourceDate === targetDate) return plan;

  const deltaDays = Math.round((new Date(targetDate + 'T00:00:00Z') - new Date(sourceDate + 'T00:00:00Z')) / 86400000);
  if (!deltaDays) return plan;

  const updated = JSON.parse(JSON.stringify(plan));
  updated.dataInicio = shiftIsoDate(updated.dataInicio, deltaDays);
  updated.dataFimAulas = shiftIsoDate(updated.dataFimAulas, deltaDays);
  updated.dataFim = shiftIsoDate(updated.dataFim, deltaDays);
  updated.dataProva = shiftIsoDate(updated.dataProva, deltaDays);
  (updated.semanas || []).forEach(function(week) {
    week.dataInicio = shiftIsoDate(week.dataInicio, deltaDays);
    week.dataFim = shiftIsoDate(week.dataFim, deltaDays);
    week.datasDescanso = (week.datasDescanso || []).map(function(date) { return shiftIsoDate(date, deltaDays); });
    (week.materias || []).forEach(function(item) {
      item.data = shiftIsoDate(item.data, deltaDays);
    });
  });
  return updated;
}

// Moves the remaining calendar one day forward without changing anything the
// student has already completed today. This is intentionally different from
// rescheduleMacroPlanFromPendingStudy: it brings tomorrow's work into today
// and pulls every later study/review/rest date along with it.
function advanceMacroPlanDay(plan, requestedDate, recordAdvance) {
  const targetDate = isoDate(requestedDate) || todayIso();
  if (!plan) return plan;

  const updated = JSON.parse(JSON.stringify(plan));
  function advanceFutureDate(value) {
    const date = isoDate(value);
    return date && date > targetDate ? addDays(date, -1) : value;
  }

  updated.dataFimAulas = advanceFutureDate(updated.dataFimAulas);
  updated.dataFim = advanceFutureDate(updated.dataFim);
  (updated.semanas || []).forEach(function(week) {
    week.dataInicio = advanceFutureDate(week.dataInicio);
    week.dataFim = advanceFutureDate(week.dataFim);
    week.datasDescanso = (week.datasDescanso || []).map(advanceFutureDate);
    (week.materias || []).forEach(function(item) {
      item.data = advanceFutureDate(item.data);
    });
  });
  if (recordAdvance !== false) {
    updated.calendarAdvanceDates = (updated.calendarAdvanceDates || []).concat(targetDate);
  }
  return updated;
}

function applyRecordedCalendarAdvances(plan, advanceDates) {
  return (advanceDates || []).reduce(function(updated, date) {
    return advanceMacroPlanDay(updated, date, false);
  }, plan);
}
...
function repairMacroPlan(plan, subjects, lessons, options) {
  ...
  const repairMode = requestedMode === PLAN_MODE_EXAM_DATE && isoDate(examDate) &&
    ...
    ? PLAN_MODE_EXAM_DATE
    : PLAN_MODE_LESSONS_PER_DAY;
  const repaired = buildCompleteMacroPlan(subjects, lessons, {
  ...
  repaired.calendarAdvanceDates = (plan && plan.calendarAdvanceDates || []).slice();
  return applyRecordedCalendarAdvances(repaired, repaired.calendarAdvanceDates);
}
...
function planNeedsRepair(plan, subjects, lessons) {
  ...
  let expected = buildCompleteMacroPlan(subjects, lessons, {
    modoPlanejamento: mode,
    aulasPorDia: lessonsPerDay,
    diasDescansoPorSemana: restDaysPerWeek,
  ...
  expected = applyRecordedCalendarAdvances(expected, plan.calendarAdvanceDates);
  const scalarFields = [
